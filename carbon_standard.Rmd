...
#calculating stocks_delta & stocks_delta_ratio
...
profile_ID<-as.factor(as.numeric(profile_ID_interactions))

# defining the unique ID of each profile, with Depth_cm
# NOT RUNING OK. R is crashing or taking toooo long ( more than 30 minutes)
# profile_ID_interactions2<-interaction(as.factor(dataset$Author_year), as.factor(dataset$AFS_description), as.factor(dataset$AFS_age_yrs),as.factor(dataset$Depth_cm))
# profile_ID2<-as.factor(as.numeric(profile_ID_interactions2))

# verifying the profile_ID file
profile_ID_interactions[11]
# profile_ID[11] # this comand takes considerable longer time to run
...
## The Cardinael_2018 papers do not have this information. An alternative is to consider the "Crop_species" and "Tree_species" collumns instead. 
...
## Ploting the profiles

```{r plot profiles}
#plotting all the profiles together - in black
...
# this loop is not working properly, returning the message: "Warning in zoo(SOC, depth):some methods for “zoo” objects do not work if the index entries in ‘order.by’ are not unique"
...
## Clustering with DBW - TSclust isn't running properly. I have reinstaled the package and call the libray. !!!!! not working
...
for(q in 2:8){
  #clustering with DBW
  clusters<-TSclust(TS_list, k=q) # Function TSclust could not been found 
  str(clusters)
  ...
  for(j in 1:length(unique_profile_clusters)){
    ...
    densities[[j]]<-kde2d(x=subset_plotting$C_stocks_delta_t_ha, y=subset_plotting$Initial_depth, n = 150, lims=c(-55,80,0,150))
...
dataset_profiles_cleaned_noerrors_rf<-dataset_profiles_cleaned_noerrors[,c("Longitude", "Latitude", "Climate_Köppen","Soil_type_WRB/FAO", "Initial_depth", "AFS_classification","C_stocks_delta_t_ha")]
...
rf1 <- randomForest(C_stocks_delta_t_ha~.,
                    ...
                    ntree = 500,
                    metric = "Accuracy")
lm1 <- lm(dataset_profiles_cleaned_noerrors_rf$C_stocks_delta_t_ha~dataset_profiles_cleaned_noerrors_rf$Latitude + dataset_profiles_cleaned_noerrors_rf$Climate_Koppen + dataset_profiles_cleaned_noerrors_rf$Longitude + dataset_profiles_cleaned_noerrors_rf$Soil_type + dataset_profiles_cleaned_noerrors_rf$Initial_depth + dataset_profiles_cleaned_noerrors_rf$AFS_classification)
